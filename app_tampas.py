...
def calcular_status(proxima_data): # Funçao que vai calcular o status das tampas
...
def buscar_tampa(): # Funçao que quando eu colocar o nome da tampa ele vai consultar e buscar lá no .csv, trazendo as informações da tampa.
    global tampa_atual
    ...
    if not encontrado:
        resultado.config(text="❌ Tampa não encontrada",
                         bg="#6c757d", fg="white")
...
def cadastrar():
    ...
    # Verificar se o código da tampa já existe
    with open(CSV_FILE, 'r', newline='', encoding='utf-8') as csvfile:
        leitor = csv.DictReader(csvfile)
        for row in leitor:
            if row['codigo'] == codigo:
                messagebox.showwarning("Aviso", f"Essa tampa '{codigo}' já existe.")
                return
...
def abrir_cadastro():
    ...
    tk.Label(janela_cadastro, text="Cadastro de Tampa",
...
def editar():
    global tampa_atual
    if not tampa_atual:
        messagebox.showwarning("Aviso", "Primeiro consulte uma tampa para editar.")
        return
    ...
    tk.Label(janela_edicao, text=f"Editar Tampa {tampa_atual['codigo']}",
             font=("Segoe UI", 18, "bold"), bg="#f1f3f6", fg="#212529").pack(pady=10)
    ...
    entrada_data_edit.insert(0, datetime.strptime(
        tampa_atual['data_ultima_limpeza'], "%Y-%m-%d").strftime("%d/%m/%Y"))
...
def gerar_relatorio():
    ...
    with open(CSV_FILE, newline='', encoding='utf-8') as csvfile:
        ...
        for linha in leitor:
            try: 
                ...
                print(f"Aviso: Data inválida encontrada para a tampa {linha['codigo']} no CSV. Ignorando esta entrada.")
                continue
    ...
    file_path = f"relatorios/relatorio_tampas_{proximo_mes:02d}_{ano_proximo}.txt"

    with open(file_path, "w", encoding="utf-8") as f:
        f.write("RELATÓRIO DE LIMPEZA DE TAMPAS\n")
        f.write(f"Referente ao mês {proximo_mes:02d}/{ano_proximo}\n")
        f.write("=" * 50 + "\n\n")

        if tampas_proximo_mes:
            for tampa in tampas_proximo_mes:
                f.write(f"Tampa: {tampa['codigo']}\n")
                f.write(f"Última Limpeza: {tampa['ultima']}\n")
                f.write(f"Próxima Limpeza: {tampa['proxima']}\n")
                f.write(f"Histórico: {tampa['historico']}\n")
                f.write("-" * 50 + "\n")
        else:
            f.write("Não há tampas com vencimento para este período.\n")
...
tk.Label(root, text="Controle de Limpeza de Tampas",
...
def focus_in(event):
    if entrada.get() == "Digite o código da tampa...":
...
btn1 = tk.Button(btn_frame, text="🔍 Consultar", command=buscar_tampa)
