    ...
    with open(CSV_FILE, "r", newline="", encoding="utf-8") as f:
        ...
        if "periodicidade_dias" not in header:
            messagebox.showinfo("Atualização do CSV",
                                ...
                                "Adicionando a coluna 'periodicidade_dias' com valor padrão de 180 dias para tampas existentes.")
...
def buscar_tampa():
    ...
    resultado_label.config(text="❌ tampa não encontrada", fg="#7f8c8d")
...
def cadastrar():
    ...
    try:
        ...
        messagebox.showerror("Erro", "Periodicidade inválida! Digite apenas o dia.")
        return

    with open(CSV_FILE, "r", newline="", encoding="utf-8") as csvfile:
        for row in csv.DictReader(csvfile):
            if row["codigo"] == codigo:
                messagebox.showwarning("Aviso", f"tampa '{codigo}' já existe.")
                return

    with open(CSV_FILE, "a", newline="", encoding="utf-8") as csvfile:
        csv.writer(csvfile).writerow([codigo, data_formatada.isoformat(), historico, str(periodicidade_dias)])

    messagebox.showinfo("Sucesso", "Tampa cadastrada com sucesso!")
...
def abrir_cadastro():
    ...
    janela_cadastro.title("➕ Cadastrar Tampa")
...
def editar_tampa_unica():
    global tampa_atual
    if not tampa_atual:
        messagebox.showwarning("Aviso", "Consulte uma tampa para editar.")
        return
    ...
    def salvar_edicao():
        ...
        with open(CSV_FILE, newline="", encoding="utf-8") as csvfile:
            ...
            for linha in reader:
                if linha["codigo"] == tampa_atual["codigo"]:
        ...
        messagebox.showinfo("Sucesso", "Tampa editada com sucesso!")
        janela_edicao.destroy()
        buscar_tampa()
     
    janela_edicao = tk.Toplevel(root)
    janela_edicao.title(f"✏️ Editar Tampa {tampa_atual['codigo']}")
    ...
    tk.Label(
        janela_edicao,
        text=f"Editar Tampa {tampa_atual['codigo']}",
        font=("Segoe UI", 18, "bold"),
        bg="#f4f6f9",
        fg="#2c3e50",
    ).pack(pady=10)
    ...
    entrada_data_edit.insert(
        0,
        datetime.strptime(tampa_atual["data_ultima_limpeza"], "%Y-%m-%d").strftime(
            "%d/%m/%Y"
        ),
    )
    ...
    entrada_hist_edit.insert(0, tampa_atual["historico"])
    ...
    entrada_periodicidade_edit.insert(0, tampa_atual.get("periodicidade_dias", "180"))
...
def abrir_edicao_em_lote():
    janela_selecao_tampas = tk.Toplevel(root)
    janela_selecao_tampas.title("✏️ Editar Várias Tampas")
    janela_selecao_tampas.configure(bg="#f4f6f9")
    janela_selecao_tampas.geometry("800x600")
    janela_selecao_tampas.transient(root)
    janela_selecao_tampas.grab_set()

    tk.Label(
        janela_selecao_tampas,
        text="Selecione as tampas para edição em lote",
        font=("Segoe UI", 18, "bold"),
        bg="#f4f6f9",
        fg="#2c3e50",
    ).pack(pady=10)

    frame_tree = tk.Frame(janela_selecao_tampas, bg="white", relief="solid", bd=1)
    ...
    def prosseguir_edicao_em_lote():
        ...
        if not selecionados:
            messagebox.showwarning("Aviso", "Nenhuma tampa selecionada para edição.")
            return

        janela_selecao_tampas.destroy()
        abrir_form_edicao_lote(list(selecionados))

    btn_selecionar = tk.Button(janela_selecao_tampas, text="Prosseguir para Edição", command=prosseguir_edicao_em_lote)
    estilo_botao(btn_selecionar, "#f39c12", "#d68910")
    btn_selecionar.pack(pady=15, fill="x", padx=20)

    janela_selecao_tampas.wait_window(janela_selecao_tampas)
    root.grab_release()

def abrir_form_edicao_lote(codigos_selecionados):
    janela_edicao_lote = tk.Toplevel(root)
    janela_edicao_lote.title(f"✏️ Editar {len(codigos_selecionados)} Tampas")
    ...
    tk.Label(
        janela_edicao_lote,
        text=f"Editar em Lote ({len(codigos_selecionados)} tampas)",
        font=("Segoe UI", 18, "bold"),
        bg="#f4f6f9",
        fg="#2c3e50",
    ).pack(pady=10)
    ...
    tk.Label(
        janela_edicao_lote,
        text="Preencha APENAS os campos que deseja alterar para todas as tampas selecionadas.\nDeixe em branco para MANTER o valor atual.",
    ...
    tk.Label(
        janela_edicao_lote,
        text="Tampas Selecionadas:",
        font=("Segoe UI", 12, "bold"),
        bg="#f4f6f9",
        fg="#2c3e50",
    ).pack(pady=(10, 0))

    text_tampas = tk.Text(janela_edicao_lote, height=5, width=40, font=("Segoe UI", 10), wrap="word", relief="solid", bd=1)
    text_tampas.pack(pady=5, padx=20, fill="x")
    for codigo in codigos_selecionados:
        text_tampas.insert(tk.END, f"- {codigo}\n")
    text_tampas.config(state=tk.DISABLED)

    scrollbar_text = tk.Scrollbar(janela_edicao_lote, command=text_tampas.yview)
    scrollbar_text.pack(side="right", fill="y")
    text_tampas.config(yscrollcommand=scrollbar_text.set)
    ...
    def aplicar_edicoes_lote():
        ...
        messagebox.showinfo("Sucesso", f"{len(codigos_selecionados)} tampas foram atualizadas com sucesso!")
        janela_edicao_lote.destroy()
        if tampa_atual and tampa_atual["codigo"] in codigos_selecionados:
            buscar_tampa() 
...
def gerar_relatorio():
    ...
    tampas_proximo_mes.sort(key=lambda x: x['proxima']) 
    if not os.path.exists("relatorios"):
        os.makedirs("relatorios")

    file_path_excel = f"relatorios/relatorio_tampas_{primeiro_dia_proximo_mes.month:02d}_{primeiro_dia_proximo_mes.year}.xlsx"
    workbook = openpyxl.Workbook()
    sheet = workbook.active
    sheet.title = "Relatório de Tampas"
    ...
    title_cell = sheet['A1']
    title_cell.value = "RELATÓRIO DE LIMPEZA DE TAMPAS"
    ...
    row_num = 5
    if tampas_proximo_mes:
        for c in tampas_proximo_mes:
        ...
        sheet['A5'].value = "Não há tampas para relatar neste período."
...
root = tk.Tk()
root.title(" Controle de Limpeza de Tampas")
root.state("zoomed")
root.configure(bg="#f4f6f9")

tk.Label(
    root,
    text=" Controle de Limpeza de Tampas",
...
menu_edicao.add_command(label="Editar Tampa Selecionada", command=lambda: verificar_senha(editar_tampa_unica))
...
btns_esquerda = [
    ("🔍 Consultar", buscar_tampa, "#34495e", "#2c3e50"),
